...
/*IR Race*/
	...
	((cum_MDRO_a / (&asian_pop + &napi_pop)) * 100000)  as IR_cum_MDRO_a "Cumulative IR MDRO in Quarter, Race: Asian/NA/PI" format=10.2,
...
/*IR Age*/
	...
	/*Pediatric age group*/
	((cum_MDRO_0017 / &ped_0017) * 100000) as IR_cMDRO_0017 "Cumulative IR CRE in Quarter: Pediatric 0-17" format=10.2,
...
select
	...
	sum (case when mdro_rural in (1) and race_new in ('White') then 1 else 0 end) as mdro_rural_wht "MDRO Rural Residency: White",
	sum (case when mdro_rural in (1) and race_new in ('Black or African American') then 1 else 0 end) as mdro_rural_blk "MDRO Rural Residency: Black or African American",
	sum (case when mdro_rural in (1) and race_new in ('Other') then 1 else 0 end) as mdro_rural_oth "MDRO Rural Residency: Other",
	sum (case when mdro_rural in (1) and race_new in ('Unknown') then 1 else 0 end) as mdro_rural_unk "MDRO Rural Residency: Unknown",
	sum (case when mdro_rural in (1) and race_new in ('Asian or Native Hawaiian or Pacific Islander') then 1 else 0 end) as mdro_rural_asian "MDRO Rural Residency: Asian/NA/PI",
	sum (case when mdro_rural in (1) and race_new in ('American Indian Alaskan Native') then 1 else 0 end) as mdro_rural_aian "MDRO Rural Residency: American Indian Alaskan Native",
	sum (case when mdro_rural in (1) and race_new in (' ') then 1 else 0 end) as mdro_rural_miss "MDRO Rural Residency: Missing",

	/*Non-rural residency by race*/
	sum (case when mdro_NONrural in (1) and race_new in ('White') then 1 else 0 end) as mdro_nonrural_wht "MDRO Non-Rural Residency: White",
	sum (case when mdro_NONrural in (1) and race_new in ('Black or African American') then 1 else 0 end) as mdro_nonrural_blk "MDRO Non-Rural Residency: Black or African American",
	sum (case when mdro_NONrural in (1) and race_new in ('Other') then 1 else 0 end) as mdro_nonrural_oth "MDRO Non-Rural Residency: Other",
	sum (case when mdro_NONrural in (1) and race_new in ('Unknown') then 1 else 0 end) as mdro_nonrural_unk "MDRO Non-Rural Residency: Unknown",
	sum (case when mdro_NONrural in (1) and race_new in ('Asian or Native Hawaiian or Pacific Islander') then 1 else 0 end) as mdro_nonrural_asian "MDRO Non-Rural Residency: Asian/NH/PI",
	sum (case when mdro_NONrural in (1) and race_new in ('American Indian Alaskan Native') then 1 else 0 end) as mdro_nonrural_aian "MDRO Non-Rural Residency: American Indian Alaskan Native",
	sum (case when mdro_NONrural in (1) and race_new in (' ') then 1 else 0 end) as mdro_nonrural_miss "MDRO Non-Rural Residency: Missing",
	...
	/*High SVI*/
	sum (case when svi in (1) and race_new in ('White') then 1 else 0 end) as svi_hi_wht "Risk Index High: White",
	sum (case when svi in (1) and race_new in ('Black or African American') then 1 else 0 end) as svi_hi_blk "Risk Index High: Black or African American",
	sum (case when svi in (1) and race_new in ('Other') then 1 else 0 end) as svi_hi_oth "Risk Index High: Other",
	sum (case when svi in (1) and race_new in ('Unknown') then 1 else 0 end) as svi_hi_unk "Risk Index High: Unknown",
	sum (case when svi in (1) and race_new in ('Asian') then 1 else 0 end) as svi_hi_asian "Risk Index High: Asian",
	sum (case when svi in (1) and race_new in ('American Indian Alaskan Native') then 1 else 0 end) as svi_hi_aian "Risk Index High: American Indian Alaskan Native",
	sum (case when svi in (1) and race_new in (' ') then 1 else 0 end) as svi_hi_miss "Risk Index High: Missing",

	/*High SVI*/
	sum (case when svi in (0) and race_new in ('White') then 1 else 0 end) as svi_lo_wht "Risk Index Low: White",
	sum (case when svi in (0) and race_new in ('Black or African American') then 1 else 0 end) as svi_lo_blk "Risk Index Low: Black or African American",
	sum (case when svi in (0) and race_new in ('Other') then 1 else 0 end) as svi_lo_oth "Risk Index Low: Other",
	sum (case when svi in (0) and race_new in ('Unknown') then 1 else 0 end) as svi_lo_unk "Risk Index Low: Unknown",
	sum (case when svi in (0) and race_new in ('Asian or Native Hawaiian or Pacific Islander') then 1 else 0 end) as svi_lo_asian "Risk Index Low: Asian/NH/PI",
	sum (case when svi in (0) and race_new in ('American Indian Alaskan Native') then 1 else 0 end) as svi_lo_aian "Risk Index Low: American Indian Alaskan Native",
	sum (case when svi in (0) and race_new in (' ') then 1 else 0 end) as svi_lo_miss "Risk Index Low: Missing"
...
select
	...
	(svi_hi_aian / &svihighaian) *100000 as svihigh_ir_aian "Risk Index Greater than or equal to 0.80 IR, Race: American Indian/ Alaska Native" format=10.2,
	(svi_hi_asian / (&svihighasian + &svihighnhpi)) *100000 as svihigh_ir_asian "Risk Index Greater than or equal to 0.80 IR, Race: Asian/NH/PI" format=10.2,
	...
	(svi_lo_asian / (&svilowasian + &svilownhpi)) *100000 as svilow_ir_asian  "Risk Index < 0.80 IR, Race: Asian/NH/PI" format=10.2,
	...
	(mdro_rural_asian / (&ruralasian + &ruralnhpi)) *100000 as rural_ir_asian "Rural residency IR, Race: Asian/NH/PI" format=10.2,
	...
	(mdro_nonrural_asian / (&nonruralasian + &nonruralnhpi)) *100000 as nonrural_ir_asian "Non-Rural residency IR, Race: Asian/NH/PI" format=10.2,
...
select
	...
	svi,
	case when density in (0) then 1 
		 when density in (1) then 0 

		 else . end as density_2,
	RACE1,
	case when density in (0) then &ruraltotalpop else 0 end as rural_population,
	case when density in (1) then &nonruralpop else 0 end as nonrural_population

from equity_race
;

create table _2x2_density as
select distinct

	case when density_2 in (1) then 1 else 0 end as rural_status,
	sum (case when density_2 in (0,1) then 1 else 0 end) as outcome_density,
	case when rural_population in (&ruraltotalpop) then (rural_population - calculated outcome_density)
		 when rural_population not in (&ruraltotalpop) then (nonrural_population - calculated outcome_density)

		 else 0 end as nooutcome_density
...
proc print data=_2x2_density noobs label;run;



data outcome_data;
    input rural_status outcome $ count;
    datalines;
0 outcome 228
0 nooutcome 6792410
1 outcome 142
...
proc freq data=outcome_data;
    tables rural_status*outcome / chisq;
    weight count;
run;



/*Frequency tables with chisq for density and svi by race among CRE infections 2024*/


proc freq data=report_statsig;
tables density_2*race_new /norow nocol nopercent nocum  fisher expected;
...
proc freq data=report_statsig;
tables svi*race_new / norow nocol nopercent nocum chisq expected;
...
proc freq data=report_statsig;
title 'SVI hi vs. low';
	tables svi/binomial(p=0.2 level='1') exact;
run;

/*1 = Rural*/
proc freq data=report_statsig;
title 'Rural vs. Non-rural res.';
	tables density_2/binomial(p=0.35 level='1');
run;
...
title 'Race= White';
	tables svi /binomial(p=0.5 level='1');
		where race_new in ('White');
run;
proc freq data=report_statsig;
title 'Race= Black or African American';
	tables svi /binomial(p=0.5 level='1');
		where race_new in ('Black or African American');
run;
proc freq data=report_statsig;
title 'Race= Other';
	tables svi /binomial(p=0.5 level='1');
		where race_new in ('Other');
run;
proc freq data=report_statsig;
title 'Race= Unknown';
	tables svi /binomial(p=0.5 level='1');
		where race_new in ('Unknown');
run;
proc freq data=report_statsig;
title 'Race= Asian';
	tables svi /binomial(p=0.5 level='1') exact;
		where race_new in ('Asian');
run;
proc freq data=report_statsig;
title 'Race= American Indian Alaska Native';
	tables svi /binomial(p=0.5 level='1');
		where race_new in ('American Indian Alaskan Native');
run;
